...
class Controller extends BaseController
{
    ...
    {
    	$name=$request->input('name');  //Taking User input
    	$phno=$request->input('phone_number');
    	$email_id=$request->input('email_id');

    	

    	if (DB::table('data')->where('email_id', $email_id)->exists())      //If the email already exists in the database then redirect to register page
    	{
    	...
    	else
    	{
    		$a=new data;
	    	$a->name=$name;
	    	$a->phone_number=$phno; //storing the data in the database
	    	$a->email_id=$email_id;
	    	$a->save();	
    ...
    {
        $email=$request->input('email_id');     //get data from user
        $phone_number=$request->input('phone_number'); 
        if(DB::table('data')->select('email_id','phone_number')->where('email_id',$email)->exists())//check if the same email exists in the database
        {
            ...
            {
                //return $user;
                return redirect()->route('tictactoe');//redirect to home page
            }
            else
            {
